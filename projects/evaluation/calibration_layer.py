...
from projects.sparsercnn import SparseRCNNDatasetMapper
from projects.evaluation.calibration_resnet import resnet101, resnet50
import pdb
...
class PrototypicalCalibrationBlock:
    ...
    def __init__(self, cfg):
        ...
        self.alpha = self.cfg.TEST.PCB.ALPHA
        self.imagenet_model = self.build_model(cfg)
        self.dataloader = build_detection_test_loader(cfg, cfg.DATASETS.TRAIN[0], mapper=SparseRCNNDatasetMapper(cfg, is_train=False))
    ...
    def build_model(self,cfg):
        if self.cfg.TEST.PCB.RES_WEIGHTS.lower() == 'none' or self.cfg.TEST.PCB.RES_WEIGHTS is None :
            logger.info("Loading Official ImageNet Pre-train Model")
            imagenet_model = resnet101(True) if cfg.TEST.PCB.DEPTH==101 else resnet50(True)
        else:
            logger.info("Loading ImageNet Pre-train Model from {}".format(self.cfg.TEST.PCB.RES_WEIGHTS))
    ...
    def execute_calibration(self, inputs, dts):
        ...
        ileft = (dts[0]['instances'].scores > self.cfg.TEST.PCB.SCORE_UPPER).sum()
        iright = (dts[0]['instances'].scores > self.cfg.TEST.PCB.SCORE_LOWER).sum()
        assert ileft <= iright
        scores,pred_classes,pred_boxes = dts[0]['instances'].scores,dts[0]['instances'].pred_classes,dts[0]['instances'].pred_boxes.tensor
        sort_score,sort_index = torch.sort(scores,descending=True)
        sort_pred_class,sort_pred_boxes = pred_classes[sort_index],pred_boxes[sort_index]
        dts[0]['instances'].scores = sort_score
        dts[0]['instances'].pred_classes = sort_pred_class
        dts[0]['instances'].pred_boxes.tensor = sort_pred_boxes
